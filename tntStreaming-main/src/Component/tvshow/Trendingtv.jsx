...
import { useGetTrendingTvQuery } from "../../app/moviApi";
          ...
          <SwiperSlide key={movie.id}>
            <section className="relative text_color movieContainer m-auto mt-8">
              ...
              <div className="absolute top-20 lg:top-16 md:top-12 sm:top-6 left-16 md:left-10 sm:left-5">
                ...
                <p className="max-w-2xl text-base md:text-sm sm:text-xs leading-6 mt-4">
                  {movie.overview}
                </p>
                <p className="mt-2 sm:text-xs">
                  Release Date - {movie.first_air_date}
                </p>
                <Link to={`/videoTv/${movie.id}`}>
                  <button className="bg-red-500 text-white px-4 py-2 rounded-md mt-4 hover:bg-sky-950 duration-300 md:text-sm sm:text-xs">
                    Watch Tralier
                  </button>
                </Link>
              </div>
